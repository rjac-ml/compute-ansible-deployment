...
- Q: How are concurrent workflow runs against the same fleet prevented? → A: GitHub Actions `concurrency` group keyed on account + region + versionmesh (format: `ansible-node-exporter-${{ inputs.account }}-${{ inputs.aws_region }}-${{ inputs.versionmesh }}`, matching the existing repo convention) with `cancel-in-progress: false`. A second run (e.g. scheduled tick during a push run) queues instead of racing. No target-side locking and no external lock store.
...
- **FR-001**: The system MUST install Prometheus Node Exporter on every EC2 instance whose tags match a configurable selector. The user-stated default selector is: `Environment=dev`, `ManagedBy=ansible`, `Process=monitoring`, `version=blue`, `Owner=<current GitHub repository name>`, and `instance-state-name=running`. All five tag values plus the environment and version slot MUST be overridable via workflow inputs without editing role or inventory code.

  **v1 implementation note (adapter selector)**: The live `ec2-extended` Terraform module today emits a different tag schema (`Project`, `Account`, `Versionmesh`, `Service`, `ManagedBy=terragrunt`, etc. — see `research.md` Finding 1). Until the follow-up sub-topic branch `003-ansible-node-exporter--align-tag-schema` adds the user-stated tags to the EC2 module, the v1 inventory MUST use the **adapter selector** defined in `research.md` §Decision 1 (`Project=compute-ansible`, `Account=dev`, `Versionmesh=bluemesh`, `Service=ec2-extended`, `instance-state-name=running`). The configurability requirement above still applies — every selector key must be overridable via workflow inputs.
...
- **FR-013**: The system MUST surface per-host failures clearly in the workflow logs (which host, which task, what error), and MUST continue running against the other hosts when one host fails. (See FR-017 for the trigger-aware exit-code semantics; FR-013 governs in-run continuation only — once Ansible has decided to keep going past the first failure, FR-017 decides whether the *workflow* exits red or green.)
...
- **FR-018**: The workflow MUST declare a GitHub Actions `concurrency` group keyed on account, AWS region, and versionmesh, in the format `ansible-node-exporter-<account>-<aws_region>-<versionmesh>` (matching the existing repo convention `<provider>-<account>-<region>-<versionmesh>` used by sibling Terragrunt workflows), with `cancel-in-progress: false`. If a second run is triggered while one is already executing for the same account+region+versionmesh, the second run MUST queue until the first completes and MUST NOT cancel or interrupt the in-flight run. The workflow MUST NOT introduce any target-side locking (no flock on hosts, no DynamoDB lock, no external lock store).
...
- **SC-005**: The same workflow scales from 1 to at least 25 target instances in `us-east-1` without configuration changes other than the tag selector, and completes a full reconcile in under 10 minutes for that fleet size. (v1 region scope: `us-east-1` only; multi-region support is deferred to a follow-up sub-topic.)
