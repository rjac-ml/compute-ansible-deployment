...
- [ ] T011 Apply the SSM staging bucket **through CI** (uses existing `just` recipes — no new tooling). Trigger: `gh workflow run aws-infra-provision.yaml -f account=dev -f region=us-east-1 -f cluster=bluemesh`. The workflow internally runs `just aws-init dev us-east-1 bluemesh` then `just aws-apply dev us-east-1 bluemesh`, which `cd aws/dev/us-east-1/bluemesh && terragrunt apply --all`, picking up `ansible-ssm-bucket/` alongside `ec2/` and `vpc/`. **Idempotency check (the test surface for CI-driven apply)**: locally run `just aws-plan dev us-east-1 bluemesh` against the converged state and confirm `0 to add, 0 to change, 0 to destroy` on the bucket unit. (Re-running the workflow itself works too but a local plan is faster and the failure mode is identical.)
- [ ] T012 (Trust root — admin-local, the one exception to CI-driven apply) Admin applies the OIDC changes via the existing recipe pair: first `just aws-plan-shared dev` (dry-run review), then `just aws-apply-shared dev` (which runs `terragrunt apply --all` under `aws/dev/shared/`). This is admin-local because the OIDC role is what CI assumes — chicken-and-egg. Immediately after apply, run `just aws-sync-oidc-secret dev` to push the new role ARN into the `GH_ROLE_DEV` GitHub Actions secret so subsequent CI runs (T011 re-runs, Phase 3 workflows) pick up the updated permissions. After this single bootstrap, no further local apply is needed for this feature.
- [ ] T013 Verify (local, after T011's CI run + T012's admin apply both complete): `aws iam list-role-policies --role-name <github-actions-role>` includes `ansible-ssm`; `aws s3 ls s3://compute-ansible-<account-id>-us-east-1-bluemesh-ansible-ssm/` succeeds (empty bucket exists). The role name comes from the OIDC module output and is the same role ARN that `just aws-sync-oidc-secret dev` just synced.
